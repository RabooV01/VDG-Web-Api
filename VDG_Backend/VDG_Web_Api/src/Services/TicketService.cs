	...
	public class TicketService : ITicketService
	{
		...
		public async Task<IEnumerable<StatisticTicketDTO>> GetTicketStatistics()
		{
			try
			{
				...
				return tickets.Where(t => t.TicketMessages.Any()).GroupBy(t => t.TicketMessages.OrderBy(t => t.Date).First().Date, (date, tickets) =>
				{
					var d = date.ToString("ddd");
					return new StatisticTicketDTO()
					{
						Date = d,
						ClosedTickets = tickets.Where(t => t.Status == TicketStatus.Closed).Count(),
						OpenTickets = tickets.Where(t => t.Status == TicketStatus.Open).Count(),
						PendingTickets = tickets.Where(t => t.Status == TicketStatus.Pending).Count()
					};
				}).ToList();
			}
			catch (Exception)
